...
class Stage3CleanupV005MigrationIT {
    ...
    void v005_cleans_up_updated_at_hibernate_sequence_and_renames_index() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(PG.getJdbcUrl(), PG.getUsername(), PG.getPassword());
            ...
            try (ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM information_schema.columns "
            ...
            stmt.executeUpdate("INSERT INTO message_log (id, guild, channel, nick_name, effective_name, message, "
        ...
        try (Connection conn = DriverManager.getConnection(PG.getJdbcUrl(), PG.getUsername(), PG.getPassword());
            ...
            try (ResultSet rs = stmt.executeQuery("SELECT indexname FROM pg_indexes "
            ...
            try (ResultSet rs =
                    stmt.executeQuery("SELECT COUNT(*) FROM pg_sequences WHERE sequencename = 'hibernate_sequence'")) {
                assertThat(rs.next()).isTrue();
                assertThat(rs.getLong(1))
                        .as("hibernate_sequence must not exist on fresh env (IF EXISTS guards absence without error)")
                        .isZero();
            }

            // 4e. New INSERT without updated_at column succeeds after V5
            stmt.executeUpdate("INSERT INTO message_log (guild, channel, nick_name, effective_name, message, "
                    + "is_converted, converted_message, channel_id) "
                    + "VALUES ('g2', 'c2', 'nk2', 'ef2', 'hi', false, null, 666)");
            try (ResultSet rs = stmt.executeQuery("SELECT COUNT(*) FROM message_log WHERE channel_id = 666")) {
                assertThat(rs.next()).isTrue();
                assertThat(rs.getLong(1))

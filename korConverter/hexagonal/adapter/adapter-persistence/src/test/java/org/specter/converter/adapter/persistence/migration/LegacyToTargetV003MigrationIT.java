...
class LegacyToTargetV003MigrationIT {
    ...
    void v003_migrates_legacy_schema_to_target_version_timestamptz_and_identity() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(PG.getJdbcUrl(), PG.getUsername(), PG.getPassword());
            ...
            stmt.executeUpdate("INSERT INTO ignore_user (id, user_id, channel_id, name, created_at, updated_at) "
        ...
        try (Connection conn = DriverManager.getConnection(PG.getJdbcUrl(), PG.getUsername(), PG.getPassword());
            ...
            try (ResultSet rs =
                    stmt.executeQuery("SELECT version, created_at, updated_at FROM ignore_user WHERE id = 100")) {
                assertThat(rs.next())
            ...
            stmt.executeUpdate("INSERT INTO ignore_user (user_id, channel_id, name, created_at, updated_at) "

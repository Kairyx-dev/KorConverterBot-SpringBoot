...
class MessageLogV004MigrationIT {
    ...
    void v004_migrates_message_log_to_text_no_default_and_timestamptz() throws Exception {
        ...
        try (Connection conn = DriverManager.getConnection(PG.getJdbcUrl(), PG.getUsername(), PG.getPassword());
            ...
            try (ResultSet rs = stmt.executeQuery("SELECT created_at, updated_at FROM message_log WHERE id = 300")) {
                assertThat(rs.next())
                        .as("message_log row id=300 must survive V4 migration")
                        .isTrue();
            ...
            try (ResultSet rs = stmt.executeQuery("SELECT message FROM message_log WHERE channel_id = 444")) {
                assertThat(rs.next())
            ...
            try (ResultSet rs = stmt.executeQuery("SELECT channel_id FROM message_log WHERE guild = 'g3'")) {
                assertThat(rs.next())

...
public class ConverterBeanAutoConfiguration {
    ...
    public AddIgnoreUserUseCase addIgnoreUserUseCase(
        ...
        return observed(
                createTxProxy(new AddIgnoreUserService(adapter, adapter, clock), AddIgnoreUserUseCase.class, txManager),
    ...
    public RemoveIgnoreUserUseCase removeIgnoreUserUseCase(
        ...
        return observed(
                createTxProxy(
                        new RemoveIgnoreUserService(adapter, adapter, clock), RemoveIgnoreUserUseCase.class, txManager),
    ...
    public CheckIgnoreUserUseCase checkIgnoreUserUseCase(
            IgnoreUserQueryAdapter queryAdapter, PlatformTransactionManager txManager, ObservationRegistry registry) {
        return observed(
    ...
    @SuppressWarnings("unchecked")
    private <T> T createReadOnlyTxProxy(T target, Class<T> iface, PlatformTransactionManager txManager) {
        var template = new TransactionTemplate(txManager);
    ...
    private <T> T observed(T target, String name, ObservationRegistry registry) {
        return (T) Proxy.newProxyInstance(
                ...
                (proxy, method, args) -> Observation.createNotStarted(name, registry)
